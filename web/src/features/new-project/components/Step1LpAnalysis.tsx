...
import { Globe, Brain, CheckCircle2, Loader2, AlertCircle, FolderInput } from "lucide-react";
import { projectNameAtom, lpUrlAtom, analysisCompleteAtom, currentStepAtom, lpAnalysisMarkdownAtom, lpStructuredContextAtom } from "../stores";
import { useAnalyzeLp, useCreateProject } from "../api";
  ...
  const handleAnalyze = () => {
    ...
    analyzeLp(lpUrl, {
      onSuccess: async (data) => {
        const structured = parseLpAnalysisMarkdown(data.markdown);
        setLpAnalysisMarkdown(data.markdown);
        setStructuredContext(structured);
        ...
        try {
          const { project } = await createProject({
            ...
            lpRawAnalysisMarkdown: data.markdown,
          });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
        <div>
          <label className="block text-slate-700 mb-2" style={{ fontSize: "14px", fontWeight: 500 }}>プロジェクト名</label>
          <div className="flex items-center gap-2 border border-gray-200 rounded-xl px-4 py-3 focus-within:border-violet-400 focus-within:ring-2 focus-within:ring-violet-100 transition-all bg-gray-50">
            <FolderInput size={16} className="text-gray-400 shrink-0" />
            <input
              type="text"
              value={projectName}
      ...
      {analysisComplete && !isPending && (
        <div className="bg-white rounded-2xl border border-emerald-100 shadow-sm p-6">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <span className="ml-auto text-slate-400" style={{ fontSize: "12px" }}>プロジェクトに保存して詳細を編集できます</span>
          </div>
          <p className="text-slate-500 text-sm mb-3">プロジェクトに保存されました。詳細画面へ移動しています…</p>
          <button
            type="button"
            onClick={() => setCurrentStep(2)}
            className="text-violet-600 hover:text-violet-700 text-sm"
          >
            このまま編集を続ける（従来の Step2）
          </button>
        </div>
      )}
    </div>
